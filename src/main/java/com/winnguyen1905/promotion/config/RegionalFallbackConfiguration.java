...
import com.winnguyen1905.promotion.secure.RegionPartition;
...
/**
 * Configuration for regional fallback mechanisms in Promotion Service.
...
public class RegionalFallbackConfiguration {
  ...
  /**
   * Regional fallback service that manages cross-region promotion data access
   */
  @Bean
  ...
  /**
   * Regional health monitor that tracks the health of regional promotion services
   */
  @Bean
  ...
  public static class SimpleCircuitBreaker {
    ...
    public <T> T execute(java.util.function.Supplier<T> operation) throws Exception {
      if (state == State.OPEN) {
        if (isTimeoutExpired()) {
          ...
          log.debug("Promotion circuit breaker transitioning to HALF_OPEN state");
        } else {
          throw new RuntimeException("Promotion circuit breaker is OPEN");
        }
      }
    ...
    private void onSuccess() {
      ...
      if (state == State.HALF_OPEN) {
        ...
        if (successes >= successThreshold) {
          state = State.CLOSED;
          successCount.set(0);
          log.info("Promotion circuit breaker recovered to CLOSED state");
        }
      }
    }
    ...
    private void onFailure() {
      ...
      if (failures >= failureThreshold) {
        state = State.OPEN;
        successCount.set(0);
        log.warn("Promotion circuit breaker opened due to {} failures", failures);
      }
    }
  ...
  /**
   * Service that handles fallback logic when regional promotion services are
   * unavailable
   */
  @Service
  @Slf4j
  public static class RegionalFallbackService {
    ...
    /**
     * Execute promotion operation with regional fallback
     */
    public <T> T executeWithFallback(RegionPartition primaryRegion,
      ...
      try {
        return primaryCircuitBreaker.execute(() -> {
          log.debug("Executing promotion {} in primary region: {}", operationName, primaryRegion);
          setCurrentRegion(primaryRegion);
          return operation.get();
        });
      } catch (Exception e) {
        log.warn("Primary region {} failed for promotion operation {}: {}",
    ...
    /**
     * Execute promotion operation in fallback region
     */
    private <T> T executeWithFallbackRegion(RegionPartition primaryRegion,
      ...
      if (fallbackRegion == null) {
        throw new RuntimeException("No fallback region available for promotion service in " + primaryRegion);
      }

      SimpleCircuitBreaker fallbackCircuitBreaker = circuitBreakers.get(fallbackRegion);

      try {
        log.info("Executing promotion {} in fallback region: {} (primary {} unavailable)",
        ...
        return fallbackCircuitBreaker.execute(() -> {
          ...
          if (cachedResult != null) {
            log.debug("Returning cached promotion result for {} from fallback region", operationName);
            return cachedResult;
          }
        ...
        log.error("Fallback region {} also failed for promotion operation {}: {}",
        ...
        T cachedResult = getCachedResult(cacheKey);
        if (cachedResult != null) {
          log.warn("Returning stale cached promotion data for {} due to regional failures", operationName);
          return cachedResult;
        }

        throw new RuntimeException("All regional promotion services unavailable for " + operationName,
            fallbackException);
      }
    }
    ...
    /**
     * Check if a region is currently available for promotion operations
     */
      ...
      CompletableFuture.runAsync(() -> {
        try {
          ...
          if (isRegionAvailable(primaryRegion)) {
            log.info("Replicating promotion data back to recovered primary region: {}", primaryRegion);
            // Implementation would replicate data back to primary region
          }
        } catch (Exception e) {
          log.warn("Failed to replicate promotion data to primary region {}: {}",
    ...
    private String generateCrossRegionCacheKey(RegionPartition region, String operationName) {
      return "promotion:cross-region:" + region.getCode() + ":" + operationName;
    }
    ...
    private <T> T getCachedResult(String cacheKey) {
      try {
        ...
        log.debug("Error retrieving cached promotion result: {}", e.getMessage());
        return null;
      }
    }
    ...
    private void cacheResultForCrossRegion(String cacheKey, Object result) {
      try {
        ...
        log.warn("Failed to cache promotion result for cross-region access: {}", e.getMessage());
      }
    }
  ...
  /**
   * Monitor that tracks the health of regional promotion services
   */
  @Service
  @Slf4j
  ...
  /**
   * Represents the health status of a regional promotion service
   */
  public static class RegionalHealthStatus {
    ...
    public String toString() {
      return String.format("PromotionRegionalHealth{region=%s, healthy=%s, status='%s', lastChecked=%s}",
